    ...
    <>
      <style>{`
        * {margin:0; padding:0; box-sizing:border-box;}
        ...
        .nav-link, .dropdown-button {display:flex; align-items:center; gap:12px; padding:10px 12px; border-radius:8px; color:rgba(255,255,255,0.8); text-decoration:none; cursor:pointer; transition: all 0.2s ease; background-color: transparent; border:none; width:100%; text-align:left; font-family:inherit;}
        .nav-link:hover, .dropdown-button:hover {background-color:${HOVER_BG}; color:white;}
        .nav-link.active {background-color:${ACTIVE_BG}; color:white;}
        .dropdown-button.active {background-color:${ACTIVE_BG}; color:white;}
          ...
          <ul className="nav-list">
            ...
            <li className="nav-item dropdown">
              <button className="dropdown-button" onClick={()=>setDropdownOpen(!dropdownOpen)}>
                ...
                <span style={{transform: dropdownOpen ? "rotate(180deg)" : "rotate(0deg)", transition:"0.3s"}}>▼</span>
              </button>
              {dropdownOpen && (
                <div className="dropdown-menu">
                  <NavLink to="/solicitacoes/list" className={({isActive})=>`dropdown-item ${isActive?"active":""}`}>Listar Solicitações</NavLink>
                  <NavLink to="/solicitacoes/create" className={({isActive})=>`dropdown-item ${isActive?"active":""}`}>Criar Solicitação</NavLink>
                </div>
              )}
            </li>
          </ul>
