...
import { Wrench, X, AlertCircle, Loader2 } from "lucide-react"
...
export default function CreateServico({ onClose, onCreated }) {
  ...
  const fetchTipos = async () => {
    try {
      ...
      setMaquinas(maquinasData.filter((t) => t.categoria === "maquina"))
      setImplementos(implementosData.filter((t) => t.categoria === "implemento"))
    } catch (error) {
